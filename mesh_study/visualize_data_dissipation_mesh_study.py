...
os.chdir("N:/vasospasm/dissipation_viscous_calculation/python_viscous_dissipation")
...
def calculate_dissipation(pinfo, case, base_size, i_vessel, ddissipation):

    len_cycle = 30
    num_cycle = 2
    onlydat, data_indices, pathwd = get_list_files_dat(pinfo, case, base_size, num_cycle)
...
case = 'baseline'
dissipation_path_baseline = "N:/vasospasm/mesh_independence_study/" + pinfo + "/" + case + "/base_size_" + base_size + "/4-results/dissipation_viscous/"
centerline_data_path = "N:/vasospasm/mesh_independence_study/" + pinfo + "/" + case + "/"
dpoints_bas = load_dict(centerline_data_path  + "points_" + pinfo + "_" + case)
ddissipation_bas = load_dict(dissipation_path_baseline + "dissipation_" + pinfo + "_" + case)

case = 'vasospasm'
dissipation_path_vasospasm = "N:/vasospasm/mesh_independence_study/" + pinfo + "/" + case + "/base_size_" + base_size + "/4-results/dissipation_viscous/"
...
for i_vessel in range(num_vessels):
    ...
    dissipation_bas, volume_bas, dissipation_per_volume_bas = calculate_dissipation(pinfo, 'baseline', base_size, i_vessel, ddissipation_bas)
    dissipation_vas, volume_vas, dissipation_per_volume_vas = calculate_dissipation(pinfo, 'vasospasm', base_size, i_vessel, ddissipation_vas)
